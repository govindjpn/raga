...
def sql_get_doc_id(doc_name): 
    select_str = "select max(doc_id) from doc_detail  where doc_name = '" + doc_name + "'" 
...
def sql_add_doc(doc_info) : 
    try : 
        insert_str = "insert into doc_detail ('doc_name', 'doc_pathname', 'doc_password', " + \
        ...
        if (doc_id := sql_get_doc_id (doc_info.doc_name)) is None: 
            raise Exception
        insert_str = "insert into doc_access ('user_id', 'doc_id', 'doc_access') " + \
...
def sql_add_doc_chunk(doc_id, chunk_id, chunk ) : 
    try : 
        insert_str = "insert into doc_chunk ('doc_id', 'chunk_id', 'chunk_text') " + \
...
def sql_add_doc_meta(doc_id, key, value) : 
    try : 
        insert_str = "insert into doc_meta ('doc_id', 'doc_meta_id', 'doc_meta_key', " + \
            " 'doc_meta_value' )" + \
...
def sql_update_summary(doc_id, summary ) : 
    try : 
        update_str = "update doc_detail set doc_summary = " + quoted(summary) + " where doc_id = " + str(doc_id) 
...
def sql_get_doc_name(doc_id): 
    select_str = "select doc_name from doc_detail  where doc_id = " + str(doc_id)  
    doc_name = sql.get_one(select_str)
    return doc_name[0]

def sql_get_doc_json(doc_id): 
    select_str = "select doc_json from doc_json  where doc_id = " + str(doc_id)  
    doc_json_txt = sql.get_one(select_str)
    doc_json = json.loads(doc_json_txt)
    return doc_json

def sql_get_doc_template(doc_id): 
    select_str = "select doc_template_id from doc_json  where doc_id = " + str(doc_id)  
    doc_template_id = sql.get_one(select_str)[0]
    select_str = "select doc_template_name from doc_template  where doc_template_id = " + str(doc_template_id)  
    doc_template_name = sql.get_one(select_str)[0]
    return doc_template_name


def sql_get_doc_detail(doc_id): 
    select_str = "select doc_name, doc_page_count, doc_type, doc_summary from doc_detail  where doc_id = " + str(doc_id)  
...
def sql_get_doc_owner(doc_id): 
    select_str = "select user_id from doc_access  where doc_id = " + str(doc_id) + " and doc_access = 'OWNER' " 
    return sql.get_one(select_str)

def sql_get_doc_meta(doc_id): 
    select_str = "select doc_meta_key, doc_meta_value  from doc_meta  where doc_id = " + str(doc_id)   
...
def sql_get_keyvalue_name(key, value): 
    select_str = "select name from doc_key_values where system = 'foi' and key = '" + key + "' and value = '" + value + "' "  
    return sql.get_one(select_str)

def sql_is_doc_active(doc_id): 
    select_str = "select doc_active_flg from doc_detail  where doc_id = '" + doc_id + "' and doc_access = 'OWNER' " 
    return sql.get_one(select_str)

def sql_get_doc_list(user_id): 
    select_str = "select doc_id from doc_access where user_id = '" + user_id + "'" 
...
def sql_get_doc_summary (doc_id : int)  -> str: 
    try : 
        select_str = "select doc_summary from doc_detail  where doc_id = " + str(doc_id)
        row =  sql.get_one(select_str)
